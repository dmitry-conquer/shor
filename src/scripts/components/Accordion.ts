...
class Accordion {
  ...
  private bindEvents(): void {
    this.buttonElements.forEach((button, index) => {
      button.addEventListener("mouseenter", () => this.handleButtonClick(index));
      button.addEventListener("mouseleave", () => this.handleButtonClick(-1));
    });
  }
  ...
  private updateUI(): void {
    this.buttonElements.forEach((button, index) => {
      ...
      const content = button.querySelector(this.selectors.content) as HTMLElement;
